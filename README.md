...
Ensuite, un des plus gros challenge n'était pas dans le développement du jeu à proprement parlé, mais tout ce qui était autour. Nous n'avons aucune expérience en modélisation 3d, ou en animation. Heureusement internet est bien fait et il est possible d'y trouver un grand nombre de ressources, mais ça nous a donc limité à des choses qui existaient. C'est peut être une des choses sur laquelle on aurait aimé mieux faire, car on a du construire un jeu autour de certaines briques n'étant pas de nous (beaucoup d'ennemis n'ont d'ailleurs pas encore de modèles 3D). Surtout que la modélisation 3D pour jeux vidéo n'a rien avoir avec celle d'un rendu 3D, car celle ci doit pouvoir être jouable et donc optimisé ce qui est la tache la plus compliquée.

Une limitation technique qui nous attriste également sont les fps, on a essayé de nombreuses méthodes différentes qui ont légèrement améliorés la fluidité du jeu, mais pas assez. La source du problème ne nous est pas inconnu, des optimisations sur Blender, car ce sont les models 3D qui font le plus lag le jeu. Sur certains petit PC le jeu a du mal à tourner efficacement et on se retrouve un peu dans un cul de sac pour optimiser encore plus notre jeu.

L'intelligence des ennemis nous a pris pas mal de temps egalement. Un premier point et non des moindres était de faire en sorte que les ennemis ne se mangent pas tout les murs du jeu, il a donc fallu réaliser un système d'évitement des obstacles à partir du model 3D de la map originel. Un choix de gameplay qui à été pris est que les ennemis se dirigent directement vers nous dès qu'ils apparaissent sur la carte, pour pouvoir avoir un jeu plus dynamique. Un autre point sur leur intelligence à été la mise en place des différents paternes que peuvent avoir les ennemis. Chaque ennemis possède sa palette d'attaque, de soutiens, et ce sur une vingtaine d'ennemis tous rencontrable au fils du jeu.
...
- Les améliorations, pour l'instant seuls les modules d'une certaine rareté ont des sprites personnalisés, il faudrait donc généraliser ça à tous, même ceux très commun. Il serait également possible d'ajouter de nouveaux objets avec de nouveaux effets.
